...
export default function Search() {
  ...
  const [articlesFound, setArticlesFound] = useState(true); // Track if articles are found
  ...
  const updateUrl = useCallback(
    (query: string, category: string | null, page: number) => {
      ...
      if (query) {
        queryinterface SectionResultAdminProps {
  query: string;
...
Params.set("query", query);
  ...
  // You can modify the logic to check if articles exist
  const checkArticlesExist = (total: number) => {
    setArticlesFound(total > 0); // Set to false if no articles are found
  };

  return (
    <div className="flex flex-col mt-24 mx-4 sm:ml-60">
        ...
        {!searchQuery && !selectedCategory ? (
          <>
            <AllArticles
              ...
              checkArticlesExist={checkArticlesExist} // Pass check function
              onPageChange={handlePageChange}
            />
            {articlesFound &&
              totalPages > 1 && ( // Only show Pagination when articles are found
                <Pagination
                  currentPage={currentPage}
