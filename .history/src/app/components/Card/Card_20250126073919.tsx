...
export default function Card({
  ...
  <Link href={url} passHref>
    <div className="relative w-full h-full"> {/* Kontainer dengan posisi relatif */}
      <Image
        src={src}
        alt={title}
        className="object-cover w-full h-full" // Menggunakan className untuk pengaturan object-fit dan ukuran
        sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" // Ukuran gambar berdasarkan viewport
        priority
      />
    </div>

    {/* Eye icon and viewers count */}
    <div className="absolute flex items-center bg-gray-900 bg-opacity-60 text-white rounded-r-full py-1 px-2">
      <FaEye size={16} />
      <span className="ml-2 text-sm">3456</span> {/* Display viewers count */}
    </div>
    ...
    <div className="absolute bottom-0 left-0 bg-primary text-gray-50 text-sm rounded-r-full py-1 pl-4 pr-6 inline-block z-10">
      {category}
        ...
        <div className="w-full sm:ml-2">
          <h2 className="mb-1 text-gray-900 font-bold text-lg md:text-xl sm:line-clamp-2 cursor-pointer hover:underline hover:text-primary">
          ...
          <Button
            onClick={onClick}
            type="button"
            classname="group inline-flex items-center h-9 rounded-full text-sm font-semibold px-3 bg-gray-300 text-gray-700 hover:bg-primary hover:text-gray-100 focus:outline-none border-none"
          >
            Read more
            <span className="ml-2">
