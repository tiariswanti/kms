...
export default function SectionResult({
  ...
  return (
    <div>
      <h1 className="text-lg md:text-xl font-semibold text-gray-800 mb-2 sm:mb-4 ml-10 md:ml-0 xl:ml-6">
        Search Result for{" "}
      ...
      {data.articles.map((article: any, index: number) => {
        return (
          <Card
            ...
            src="/pict1.jpg"
            url={`/article/${article.source.id}/${createSlug(article.title)}`}
            onClick={() =>
              router.push(
                `/article/${article.source.id}/${createSlug(article.title)}`
              )
            }
