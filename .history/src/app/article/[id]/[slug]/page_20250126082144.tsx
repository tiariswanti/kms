...
export default function ArticlePage({ params }: ArticlePageProps) {
  ...
  const [viewSent, setViewSent] = useState(false); // Untuk memastikan view hanya dikirim sekali
    ...
    if (article && viewerId && !viewSent) {
      axios
        ...
        .then((response) => {
          ...
          setViewSent(true); // Tandai view telah dikirim
        })
        .catch((error) => {
          console.error("Failed to count view:", error);
        });
    }
  }, [article, viewerId, viewSent]); // Tambahkan viewSent ke dependensi
